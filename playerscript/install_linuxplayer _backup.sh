...
echo "#!/bin/bash" > RestartPlayer.sh
echo "export XUGGLE_HOME=/usr/local/xuggler" >> RestartPlayer.sh
echo "export LD_LIBRARY_PATH=\$XUGGLE_HOME/lib:\$LD_LIBRARY_PATH" >> RestartPlayer.sh
echo "export PATH=\$XUGGLE_HOME/bin:\$PATH" >> RestartPlayer.sh
echo "cd ~/Desktop/dist" >> RestartPlayer.sh
echo "java -Dvlcj.check=no -Xms512M -Xmx512M -jar dmbplayer.jar" >> RestartPlayer.sh
